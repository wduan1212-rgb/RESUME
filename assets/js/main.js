(function () {
  ...
  const byId = (id) => works.find((work) => work.id === id) || works[0];
  ...
  function renderTimeline() {
    ...
    const dragon2025 = content2025.filter((work) => work.displayGroup === "timeline-2025-dragon");
    ...
    root.innerHTML = `
      ...
      ${yearBlock("2025", "Visual Exploration", "Dragon Covenant / Competition Boards", `
        ...
        <div class="dragon-row">
          ${dragon2025.map((work) => archiveItem(work)).join("")}
        </div>
  ...
  function setupHeroVideo() {
    ...
    if (video.src) playWithPreferredSound();
    updateLabel("on");
  ...
  function setupModal() {
    ...
    const open = (id) => {
      ...
      if (work && work.type === "exhibition-board" && work.coverUrl) {
        ...
        body.innerHTML = `<video class="embed-slot" src="${escapeHTML(work.videoUrl)}" controls playsinline></video>`;
        ...
        body.innerHTML = `<video class="embed-slot" src="${escapeHTML(site.heroVideo)}" controls playsinline></video>`;
      } else if (id === "intro" && site.introVideo) {
        body.innerHTML = `<video class="embed-slot" src="${escapeHTML(site.introVideo)}" controls playsinline></video>`;
      } else {
        body.innerHTML = `
